...
#define _SUB_RSP_ARG_(num) \
                    ...
                    _WRITE_VAL_THR_(0x48, 0x83, 0xec)\
                    _WRITE_VAL_ONE_(num)
...
#define _MUL_RCX_\
                    ...
                    _WRITE_VAL_THR_(0x48, 0xf7, 0xe1)

#define _DIV_RCX_\
                    fprintf (file_output, "div rcx\n");\
                    _WRITE_VAL_THR_(0x48, 0xf7, 0xf1)
...
static int make_asm (Asm_values* asm_st, Tree* tree_node, Variables* var_arr)
{
    ...
    cur_amount_var = 0;
    IsDifTree (tree_node);
    int save_am_var = cur_amount_var;

    asm_st->ip = START_EXECUTE_CODE;

    _PUSH_RPB_
    _MOV_RBP_RSP_
    _SUB_RSP_ARG_(SIZEOF_DATA * cur_amount_var)
    cur_amount_var = 0;
    ...
    _PUSH_RPB_
    _MOV_RBP_RSP_
    _SUB_RSP_ARG_(SIZEOF_DATA * save_am_var)
    make_asm_recurse (asm_st, tree_node, var_arr);
    _MOV_RSP_RBP_
    _POP_RBP_
    _MOV_RAX_ARG_(1)
...
#define _JAE_\
        ...
        _WRITE_VAL_ONE_(0x0f)\
        _WRITE_VAL_ONE_(0x83)

#define _JBE_\
        fprintf (output_file, "jbe ");                        \
        _WRITE_VAL_ONE_(0x0f)\
        _WRITE_VAL_ONE_(0x86)

#define _JB_\
        fprintf (output_file, "jb ");                        \
        _WRITE_VAL_ONE_(0x0f)\
        _WRITE_VAL_ONE_(0x82)

#define _JA_\
        fprintf (output_file, "ja ");                        \
        _WRITE_VAL_ONE_(0x0f)\
        _WRITE_VAL_ONE_(0x87)
